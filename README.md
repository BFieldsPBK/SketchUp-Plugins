...
one shared **BSF Office Tools** toolbar (and a matching submenu under
`Extensions > BSF Office Tools`), so everything stays gathered in one place.

## Plugins

| Plugin | What it does |
| --- | --- |
| `bsf_suite` | The shared toolbar/menu hub. No commands of its own — it hosts every other plugin's buttons. |
...
| `field_designer` | Regulation soccer and football fields by age group, optionally wrapped in a regulation 400 m or 300 m running track. |

Each plugin also works standalone: if `bsf_suite` isn't installed, it falls
back to its own toolbar, exactly as before.

### Field Designer

One **Field Generator** button opens a dialog with:

- **Soccer presets** — U6 and U8 (4v4), U10 (7v7, with build-out lines),
  U12 (9v9), High School (NFHS 110×65 yd), College (NCAA 115×75 yd), and
...
- **Football presets** — Youth (80-yard field) and Regulation
  (HS/College/Pro, 100 yd + two 10 yd end zones × 160 ft), with yard lines
  every 5 yards.
- **Track wrap** — None, 400 m (8 lanes, World Athletics standard: 36.50 m
  kerb radius, 84.39 m straights), 300 m (6 lanes, compact configuration),
  or **Auto**, which picks the smallest regulation track whose infield
  holds the field with 2 m of clearance — 300 m for small-sided fields,
  400 m for full-size soccer and football.
- **Overrides** — soccer length/width within a governing body's legal
  range, and the apron margin used when no track is drawn.

Output is one named group per run (surface, markings, and track as
...
verify against current rulebooks for competition work.

## Installing

Build the installers, then use `Extension Manager > Install Extension` in
SketchUp for each `.rbz` (install `bsf_suite` plus whichever tools you want):
...
- For a fast loop, symlink (or copy) a plugin's contents into your
  ...
  SketchUp.
- New plugins join the shared toolbar by requiring `bsf_suite/registry`
  and calling `BSF::Suite::Registry.add_commands("My Tool", commands)` —
...
- Field Designer: hash marks / yard numbers for football, lane stagger
  markings and D-zones for the track, 3D goals and goal posts.
